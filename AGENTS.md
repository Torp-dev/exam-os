...
- STUDIO Sep 24: `npm create sanity` scaffolder failed (ENOENT on studio/package.json even after mkdir; --help hung) → hand-wrote minimal `studio/` (package.json pinned sanity 6.16.0 + react 19, sanity.config.ts importing shared `../src/sanity/schemas`, sanity.cli.ts). Single schema source of truth. Deleted obsolete root `sanity.config.ts` (embedded route already gone). CLI authed as Google user; `studio/` deps installing.
